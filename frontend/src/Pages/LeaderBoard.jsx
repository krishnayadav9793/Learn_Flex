...
const Leaderboard = () => {
  ...
  const TableRow = ({ user, isCurrentUser = false }) => {
    ...
    return (
      <div
        className={`grid items-center px-5 py-3 border-b border-[#D6E6F4] last:border-0 transition-all duration-150
          ${isCurrentUser
            ? "bg-[#0B2447]"
            : isTop3
            ? "bg-[#F7F9FC]"
        ...
        style={{ gridTemplateColumns: "56px 1fr 80px 80px 80px 90px" }}
      >
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <span className={`text-sm font-bold truncate block ${isCurrentUser ? "text-white" : "text-[#0B2447]"}`}>
              {user.name}
              {isCurrentUser && (
                <span className="ml-2 text-[9px] font-black bg-white/20 text-white px-1.5 py-0.5 rounded-full uppercase tracking-wider">
                  You
                </span>
              )}
            </span>
        ...
        <div className="flex flex-col items-end gap-1">
          <span className={`text-sm font-black ${isCurrentUser ? "text-white" : "text-[#0B2447]"}`}>
            {user.rating}
          </span>
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#EAF3FB] via-[#F7F9FC] to-white p-6 font-sans">
      <div className="max-w-3xl mx-auto flex flex-col gap-6">
        ...
        <div className="relative bg-white border border-[#D6E6F4] rounded-[2rem] px-8 py-8 text-center shadow-lg shadow-blue-900/8 overflow-hidden">
          ...
          <p className="relative text-sm text-slate-400 font-medium">{quizName ?? "Top performers"}</p>
        </div>
        ...
        {/* ── YOUR RANK CARD — sits between podium and table ── */}
        {!loading && currentUserRow && (
          <div className="relative bg-[#0B2447] rounded-2xl overflow-hidden shadow-lg shadow-blue-900/30 border border-[#1a3a6b]">
            {/* top accent */}
            ...
            {/* reuse the same TableRow with dark styling */}
        ...
        <div className="bg-white border border-[#D6E6F4] rounded-2xl overflow-hidden shadow-md shadow-blue-900/6">
          {/* Header */}
          <div
            className="bg-[#F7F9FC] border-b border-[#D6E6F4] px-5 py-3 grid items-center"
            style={{ gridTemplateColumns: "56px 1fr 80px 80px 80px 90px" }}
          >
            ...
            <span className="text-[10px] font-bold text-[#1a5276] uppercase tracking-widest text-center">✓ Right</span>
            <span className="text-[10px] font-bold text-[#1a5276] uppercase tracking-widest text-center">✗ Wrong</span>
            <span className="text-[10px] font-bold text-[#1a5276] uppercase tracking-widest text-center">— Skip</span>
        ...
        {!loading && currentUserId && !currentUserRow && (
          <div className="bg-white border border-[#D6E6F4] rounded-2xl px-6 py-4 text-center shadow-sm">
